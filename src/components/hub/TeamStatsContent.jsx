...
export default function TeamStatsContent() {
  ...
  const handleSearch = async (query) => {
    ...
    try {
      ...
      if (!resp.ok) throw new Error(result.error || 'Team not found. Try using the full team name.');
      if (!result?.team_name) throw new Error('Team not found. Try the full team name.');

      recordLookup();
      queryClient.invalidateQueries({ queryKey: ['currentUser'] });
      setCurrentTeam(result);
    } catch (err) {
      console.error('[TeamStatsContent]', err.message);
      if (err?.message?.includes('free_limit_reached') || err?.status === 429) {
        setShowLimitModal(true);
      } else {
        setError(err.message || 'Failed to analyze team. Please try again.');
      }
    } finally {
        ...
        <div className="flex items-center justify-center py-16">
          <div className="text-center">
            <div className="relative w-16 h-16 mx-auto mb-4">
              <div className="absolute inset-0 bg-gradient-to-r from-orange-500 to-red-600 rounded-full opacity-20 animate-ping" />
              <div className="absolute inset-2 bg-slate-800 rounded-full flex items-center justify-center shadow-lg border border-orange-500/30">
