...
import { Sparkles, TrendingUp, AlertTriangle, CloudRain, Activity, Target, RefreshCw, Settings, Heart } from "lucide-react";
...
function FeedsContent() {
  ...
  const { data: brief, isLoading, error } = useQuery({
    queryKey: ['personalizedFeed', today, currentUser?.email],
    queryFn: async () => {
      if (!currentUser?.email) return null;
      
      const briefs = await base44.entities.BettingBrief.filter(
        { brief_date: today, created_by: currentUser.email },
        '-created_date',
        1
      );
  ...
  const hasPreferences = currentUser?.favorite_sports?.length > 0 || currentUser?.favorite_leagues?.length > 0;
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            {hasPreferences 
              ? `AI-powered insights for your favorite sports: ${currentUser.favorite_sports?.map(s => s.toUpperCase()).join(', ')}`
              : 'AI-powered insights and top picks for today'
            }
          </p>
        </div>
        ...
        {!hasPreferences && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
            <Card className="border-2 border-blue-200 mb-6 bg-gradient-to-r from-blue-50 to-indigo-50">
              <CardContent className="p-6">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <h3 className="text-lg font-bold text-gray-900 mb-2">🎯 Personalize Your Feed</h3>
                    <p className="text-gray-700 mb-4">
                      Set your favorite sports and leagues in preferences to get a customized daily feed with picks, injuries, and insights tailored specifically to what you follow!
                    </p>
        ...
        {/* Focused Sports Display */}
        {brief && (brief.focused_sports?.length > 0 || brief.focused_leagues?.length > 0) && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <Card className="border-2 border-purple-200 mb-6">
            <CardContent className="p-8 text-center">
              ...
              <p className="text-gray-600 mb-6">
                {hasPreferences
                  ? `Generate your personalized feed with picks, injuries, and insights for ${currentUser.favorite_sports?.map(s => s.toUpperCase()).join(', ')}`
                  : 'Generate today\'s AI-powered feed with top picks, injury updates, and sharp money indicators'}
              </p>
              <Button
        ...
        {generateBriefMutation.isPending && (
          <Card className="border-2 border-purple-200 mb-6">
            <CardContent className="p-8 text-center">
              ...
              <h3 className="text-xl font-bold text-gray-900 mb-2">Generating Your Feed...</h3>
              <p className="text-gray-600">
                {hasPreferences
                  ? `Analyzing today's games for ${currentUser.favorite_sports?.map(s => s.toUpperCase()).join(', ')}...`
