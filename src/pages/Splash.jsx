...
export default function Splash() {
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      // Normalize keys — native bridge may use different casing
      const identityToken =
        result.identityToken ||
        result.identity_token ||
        result.idToken ||
        result.id_token ||
        null;
      ...
      if (!identityToken) {
        console.error("[Splash] Apple result missing identityToken. Keys:", Object.keys(result));
        setError("Sign in failed — missing token. Please try again.");
        return;
      }
      ...
      if (data?.success) {
        ...
        if (userToStore.full_name && !userToStore.full_name.startsWith("Apple_") && !userToStore.full_name.includes("privaterelay")) {
          ...
          localStorage.setItem("swh_full_name", typeof fullName === "string" ? fullName : [fullName.givenName, fullName.familyName].filter(Boolean).join(" "));
        }
        ...
        console.error("[Splash] handleAppleSignIn error:", data?.error);
        setError(data?.error || "Sign in failed. Please try again.");
      }
    } catch (err) {
      console.error("[Splash] Apple Sign In exception:", err);
      setError("Sign in failed. Please try again.");
    } finally {
